...
def test_day_counter_is_opt_in_and_generic(monkeypatch):
    gateway = importlib.import_module("gateway")
    monkeypatch.delenv("ANCHOR_DAY_COUNTERS", raising=False)
    monkeypatch.delenv("ANCHOR_TIMEZONE_OFFSET", raising=False)
    assert gateway.get_day_counter() == ""

    monkeypatch.setenv(
        "ANCHOR_DAY_COUNTERS", '{"synthetic milestone":"2026-01-01"}',
    )
    monkeypatch.setenv("ANCHOR_TIMEZONE_OFFSET", "0")
    assert "synthetic milestone: day " in gateway.get_day_counter()
