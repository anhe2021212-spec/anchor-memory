...
CREATE TABLE IF NOT EXISTS belief_constellations (
 ...
 created TEXT NOT NULL, created_by TEXT NOT NULL DEFAULT 'agent',
...
def _case_row(row) -> dict:
    return {
        ...
        "emotion_score": float(row["emotion_score"]), "created_by": row["created_by"] or "agent",
            ...
            for case_kind, key in (("support", "support_cases"), ("contradiction", "contradiction_cases"), ("boundary", "boundary_cases")):
                for ordinal, case in enumerate(belief.get(key, []) or []):
                    ...
                    conn.execute("INSERT INTO belief_cases VALUES(?,?,?,?,?,?,?,?,?,?)", (
                        ...
                        float(case.get("emotion_score", 0.5)), case.get("created_by", "agent"),
                    ))
...
def replace_data(db, data: dict) -> None:
    ...
    with db._conn() as conn:
        ...
        for cid, (bid, kind, case) in desired_cases.items():
            ...
            values = (bid, kind, memory_id, inline_text, str(case.get("weight_note") or "").strip(), case.get("occurred_at"), case.get("added") or datetime.now().date().isoformat(), float(case.get("emotion_score", 0.5)), case.get("created_by", "agent"))
        ...
        for cognition_id, bid in desired_constellations:
            ...
            conn.execute("INSERT OR IGNORE INTO belief_constellations VALUES(?,?,0.0,?,?)", (cognition_id, bid, datetime.now().date().isoformat(), "agent"))
        conn.commit()
