...
2026-06-17 B窗口建, 并在服务器实机校准(BAAI/bge-base-zh-v1.5)。校准表见文末。
...
_PROTOS = {
    ...
    "none": [
        "我现在肚子疼", "我好累啊", "我有点不舒服", "我头好疼",
        "在吗", "早上好", "晚安", "我回来了",
        "帮我跑一下这个", "把这个改一下", "你刚才说什么", "嗯嗯好的",
    ],
    # 回忆、关系史、专有名词、事实查询: 该翻记忆
    "full": [
        "你还记得我们上次说的吗", "你记得那件事吗", "我们当时是怎么决定的",
        "关于AI agent的系统架构", "回游录里是怎么写的", "那个人是谁来着",
    ],
}

# 显式回忆/事实线索词: 命中直接放行 full, 不看向量分。
# 专治"我肚子疼跟上次那次一样吗"这种"即时句壳+回忆芯"复合句(校准时这句一度被误杀)。
...
# cheap guard: 纯 sticker/省略号/标点符号/短拟声撒娇词, 不值得走向量检索。
...
_SOFT_NOISE = {
    ...
    "嗯", "嗯嗯", "嗯哼", "好耶", "贴贴", "亲亲", "抱抱", "摸摸", "蹭蹭",
}
_ADDRESS_RE = re.compile(r"^(伙伴|朋友|老师|宝宝|宝贝|AI agent)+")
...
def _cheap_none(query: str) -> str:
    ...
    if not sem:
        return "纯标点/表情·不翻记忆"
    # 很短的拟声/撒娇词, 允许前面带一个称呼；避免把正经短问句误杀。
