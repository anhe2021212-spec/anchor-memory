...
_ACTION_PATTERNS = {
    "companionship": re.compile(r"陪|在这|在场|留在|听我说"),
    "presence": re.compile(r"陪|在这|还在|不走|留在"),
    "continuity": re.compile(r"好久没继续|重新联系|又见面|继续聊"),
    "reconnection": re.compile(r"回来|重新联系|又见面"),
    "supportive_observation": re.compile(r"有帮助|清楚|支持|鼓励"),
    "shared_activity": re.compile(r"一起讨论|继续聊|一起完成"),
    "request_presence": re.compile(r"陪|在吗|还在|聊聊天|留在"),
    "anxiety": re.compile(r"焦虑|紧张|恐慌|手抖"),
    "fear": re.compile(r"害怕|怕|恐惧"),
    "support_request": re.compile(r"陪|帮助|支持|听我说|在这"),
    "work": re.compile(r"工作|上班|值班|下班"),
    "irregular_schedule": re.compile(r"熬夜|值班|凌晨|深夜|作息"),
    "prior_topic": re.compile(r"上次|之前|讨论|说过|方案"),
    "purchase_completed": re.compile(r"买了|购买|到货|下单"),
    "design_decision": re.compile(r"设计方案|架构方案|决定|取舍"),
    ...
    "named_subject_relation": re.compile(r"关系|朋友|协作|认识|信任|不在"),
    "legacy_authorship": re.compile(r"写过|留下|做过|意图路由|maintainer"),
    "recurring_life_event": re.compile(r"又|再次|坏了|失灵|出问题"),
}
