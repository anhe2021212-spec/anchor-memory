...
def enabled() -> bool:
    return os.environ.get("ANCHOR_DUAL_EDGE", "off").strip().lower() in {"1", "on", "true", "yes"}
...
CREATE TABLE IF NOT EXISTS semantic_edges (
 ...
 created_by TEXT NOT NULL DEFAULT 'heng',audit_note TEXT NOT NULL DEFAULT '',created TEXT NOT NULL,
    ...
    with db._conn() as conn:
        ...
        for r in rows:
            ...
            if kind in {"lateral","backfill","temporal"}:
                ...
                conn.execute("""INSERT OR IGNORE INTO semantic_edges(edge_id,source_id,target_id,role,strength,conductance,confidence,valid_from,valid_to,provenance,review_state,created_by,audit_note,created) VALUES(?,?,?,?,?,0,1,NULL,NULL,?,'approved','heng',?,?)""",
